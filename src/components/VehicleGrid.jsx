...
const VehicleGrid = ({ vehicles, onEdit, onDelete, isAdmin = false, onBulkAction }) => {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      await apiService.bulkDeleteVehicles(vehicleIds);
      setSelectedVehicles(new Set());
      if (onBulkAction) {
  ...
  const handleBulkStatusUpdate = async (newStatus) => {
    ...
    try {
      const vehicleIds = Array.from(selectedVehicles);
      const updatePromises = vehicleIds.map(id => 
        apiService.updateVehicle(id, { status: newStatus })
      );
  ...
  // Safely handle the vehicles prop
  if (!vehicles || !Array.isArray(vehicles) || vehicles.length === 0) {
    console.log('📊 VehicleGrid: No vehicles to display');
    return (
